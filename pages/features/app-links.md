...
App Links route directly to your app when opened, bypassing the web browser and a URL scheme typically used for the redirection process. They were introduced with Android M, and can result in a more seamless experience for your users.

{% protip title="Looking for iOS support?" %}
App Links are an Android-only feature, but [Universal Links]({{base.url}}/features/universal-links) are the iOS equivalent.
{% endprotip %}

Branch makes it simple to enable App Links, and even improves on them since you still get all the other benefits of Branch links:
...
1. Paste the copied fingerprint value into the **SHA256 Cert Fingerprints** field that appears.
1. Scroll down and click `Save`.

{% image src='/img/pages/features/app-links/enable_app_links.png' 3-quarters center alt='enable app links' %}
 
{% protip title="Using multiple fingerprints" %}
You can insert your debug and production fingerprints for testing. Simply separate them with a comma.
{% endprotip %}
...
1. If successful, your app should immediately open.
