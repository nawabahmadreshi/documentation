...
1. Find your email link whitelabeled domain. This is your custom tracking domain (e.g. click.branch.io)
1. Find the ESP domain. This is the ESP's host click domain (e.g. click.espdomain.net)

Provide both the click tracking domain and the ESP domain to your Branch Account Manager.
...
In order for your ESP to know that the Universal Link should open the app, add `universal="true"` to the HTML, for example:
...
To use this script, make sure you've set up deep links according to one of the [linking schemas outlined here](/third-party-integrations/integrating-your-esp/advanced/#setting-up-your-link-schema-for-email).
...
module.exports = function(original_url, branch_base_url, branch_hmac_secret, three_p_url) {
    ...
    if (!branch_base_url) { return new Error('Missing branch_base_url, should be similar to https://bnc.lt/abcd/3p?%243p=e_eml'); }
...
The Branch marketing link creator will also scrape your web URL for deep link data to make link creation even easier.
{% endprotip %}
